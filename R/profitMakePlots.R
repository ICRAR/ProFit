...
profitMakePlots <- function(image, model, region, error, errischisq = FALSE,
  cmap = rev(rainbow(1e3,end=2/3)), errcmap=cmap, plotchisq=FALSE, dofs=c()) {
  ...
  else
  {
    ...
    parmar = par("mar")
    parmar2=c(1.5,1.5,1,0.7)
    par(mar=parmar2)
    # Four images, four colour bars, two histograms
    # The model and data colour bars are identical here... oh well
    split.screen(rbind(
      c(marx, imgx-marx, 1-imgy+mary, 1-mary),
      c(imgx+marx, 2*imgx-marx, 1-imgy+mary, 1-mary),
      c(marx, imgx-marx, cbary+mary, cbary+imgy-mary),
      c(imgx+marx, 2*imgx-marx, cbary+mary, cbary+imgy-mary),
      c(marx, imgx-marx, 1-imgy-cbary+mary, 1-imgy-mary),
      c(imgx+marx, 2*imgx-marx, 1-imgy-cbary+mary, 1-imgy-mary),
      c(marx, imgx-marx, mary, cbary-mary),
      c(imgx+marx, 2*imgx-marx, mary, cbary-mary),
      c(2*imgx+marx, 1-marx, histy+mary, 1-mary),
      c(2*imgx+marx, 1-marx, mary, histy-mary)))
    screennum = 1
    ...
    screen(screennum); screennum = screennum + 1
    magimage(Data,stretchscale=stretchscale,stretch=stretch,lo=minimg,hi=maximg,type='num',zlim=zlims,col=cmap)
    tempcon=magimage(1-region,add=T,col=NA)#hsv(s=0,alpha=0.5)
    contour(tempcon,add=T,drawlabels = F,levels=1)
    legend('topleft',legend='Data')
    screen(screennum); screennum = screennum + 1
    magimage(model,stretchscale=stretchscale,stretch=stretch,lo=minimg,hi=maximg,type='num',zlims=zlims,col=cmap)
    contour(tempcon,add=T,drawlabels = F,levels=1)
    legend('topleft',legend='Model')
    screen(screennum); screennum = screennum + 1
    magimage(residual,stretchscale=stretchscale,stretch=stretch,lo=-maximg,hi=maximg,type='num',zlim=zlims,col=errcmap)
    contour(tempcon,add=T,drawlabels = F,levels=1)
    legend('topleft',legend='Data-Model')
    ...
    screen(screennum); screennum = screennum + 1
    scalemar = c(1.25,2,0.25,1)
    par(mar=scalemar)
    pady=-1
    #breaks=sinh(seq(asinh(minimg*stretchscale),asinh(maximg*stretchscale),length.out=length(cmap)+1))/stretchscale
    breaks = seq(minimg,maximg, length.out=length(cmap)+1)
    profitImageScale(image,zlims, col=cmap, breaks=breaks, axis.pos=1,axis.padj=pady)
    screen(screennum); screennum = screennum + 1; par(mar=scalemar)
    profitImageScale(model,zlims, col=cmap, breaks = breaks, axis.pos=1,axis.padj=pady)
    screen(screennum); screennum = screennum + 1; par(mar=scalemar)
    #breaks=sinh(seq(asinh(-maximg*stretchscale),asinh(maximg*stretchscale),length.out=length(cmap)+1))/stretchscale
    breaks = seq(-maximg,maximg, length.out=length(cmap)+1)
    profitImageScale(residuals,zlims, col=errcmap, breaks = breaks, axis.pos=1,axis.padj=pady)
    screen(screennum); screennum = screennum + 1; par(mar=scalemar)
    #breaks=tan(seq(atan(minerr*stretcherr),atan(maxerr*stretcherr),length.out=length(cmap)+1))/stretcherr
    breaks = seq(minerr,maxerr, length.out=length(cmap)+1)
    profitImageScale(errmap,zlims, col=errcmap, breaks = breaks, axis.pos=1,axis.padj=pady)
