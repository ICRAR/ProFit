...
inline float f_evaluate_king(float x, float y, float box, float rc, float rt, float a) {
...
kernel void king_float(
	...
	float box, float rc, float rt, float a) {
	...
	if( rscale_max > 0 && (r_prof/rscale) > rscale_max ) {
		...
		image[i] = f_evaluate_king(x_prof, y_prof, box, rc, rt, a);
#if __OPENCL_C_VERSION__ <= 120
		to_subsample[i].x = -1;
#endif /* __OPENCL_C_VERSION__ */
	}
...
kernel void king_subsample_float(
	...
	val = f_evaluate_king(x_prof, y_prof, box, rc, rt, a);
	testval = f_evaluate_king(x_prof, fabs(y_prof) + fabs(delta_y_prof), box, rc, rt, a);
	...
	private float acc_scale = log10(fabs(log10(r)))/rc/2;
