R"===(
/**
 * Double-precision Moffat profile OpenCL kernel implementation for libprofit
 *
...
inline double d_evaluate_moffat(double x, double y, double box, double rscale, double con) {
	private double r = pow(pow(fabs(x), 2+box) + pow(fabs(y), 2+box), 1/(2+box));
	private double r_factor = r/rscale;
	return pow(1 + r*r/(rscale*rscale), -con);
}

kernel void moffat_double(
	global double *image,
	...
	if( rscale_max > 0 && (r_prof/rscale) > rscale_max ) {
		...
		image[i] = d_evaluate_moffat(x_prof, y_prof, box, rscale, con);
#if __OPENCL_C_VERSION__ <= 120
		to_subsample[i].x = -1;
#endif /* __OPENCL_C_VERSION__ */
	}
...
kernel void moffat_subsample_double(
	...
	val = d_evaluate_moffat(x_prof, y_prof, box, rscale, con);
	testval = d_evaluate_moffat(x_prof, fabs(y_prof) + fabs(delta_y_prof), box, rscale, con);
