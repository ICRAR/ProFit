---
title: "Profit-Segim-Maps"
author: "Aaron Robotham"
date: "3 May 2017"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ProFit: Segim Maps}
...
segim_new = profitMakeSegim(image, plot=TRUE)
profitSegimPlot(image, segim_old)
```
...
```{r, fig.width=5, fig.height=5}
segim_new_expand=profitMakeSegimExpand(image, segim_new$segim, skycut=-1, expandsigma=5, dim=c(31,31), expand=1, plot=TRUE)
profitSegimPlot(image, segim_old)
```

This expanded segmentation map is now very similar to the hand-crafted one included with the package.

A harder example is galaxy G278109:
...
```{r, fig.width=5, fig.height=5}
segim_new = profitMakeSegim(image, tolerance = Inf, plot=TRUE)
```
...
```{r, fig.width=5, fig.height=5}
segim_new_diff = profitMakeSegim(profitImDiff(image), plot=TRUE)
```
...
profitSegimPlot(image, segim_new$segim)
```
