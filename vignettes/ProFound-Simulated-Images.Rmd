...
To see whether we have removed (or subtracted out in the case of the sky) all structure we can look at pixel-to-pixel correlation and the 2D FFT (both returned as part of the **profitPixelCorrelation** function):

```{r, fig.width=6, fig.height=6, dpi=40}
magimage(profitPixelCorrelation(im_test, plot=TRUE)$fft); points(500,500,cex=5,col='red')

magimage(profitPixelCorrelation(im_test_sub, plot=TRUE)$fft); points(500,500,cex=5,col='red')

magimage(profitPixelCorrelation(im_test_fin, plot=TRUE)$fft); points(500,500,cex=5,col='red')

magimage(profitPixelCorrelation(matrix(rnorm(1e6, mean=0, sd=10), 1000), plot=TRUE)$fft); points(500,500,cex=5,col='red')
```
