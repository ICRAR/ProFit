...
The way **ProFound** handles the various inputs allows for great flexibility and power. AS an example, it is fairly trivial to run it in a mode where you can extract multiband photometry in pixel matched data. AS a pseudo code example imagine you have 3 pixel matched 2k*2K J/H/K band images. You can use the objects detected in, say, the K band to extract photometry in the others:
...
Given these outputs a photometrically driven user can create a S/N stacked image using the **skyRMS**:

```R
stack=matrix(0,2000,2000) #Since our images are assumed to be 2k*2k
stack=stack+Jim/(pro_J_tot_dil$skyRMS^2)
stack=stack+Him/(pro_H_tot_dil$skyRMS^2)
stack=stack+Kim/(pro_K_tot_dil$skyRMS^2)
sky_stack=pro_J_tot_dil$skyRMS^2 + pro_H_tot_dil$skyRMS^2 + pro_K_tot_dil$skyRMS^2
