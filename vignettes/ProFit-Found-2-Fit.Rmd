...
In this vignette we will demonstrate fitting in the most automated mode available in \code{ProFit}.
...
```{r eval=evalglobal}
example_Bpsf = profitDoFit(image=image, loc=c(178,178), Ncomp=1.5, psf=stars_auto$psf, magzero=30, 
                           SBdilate=2)
```
...
example_B4 = profitDoFit(image=image, loc=c(178,178), Ncomp=2, psf=stars_auto$psf, magzero=30, 
                         SBdilate=2, bulge_nser_fit=FALSE, seed=1)
```
