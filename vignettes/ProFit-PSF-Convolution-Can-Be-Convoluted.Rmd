...
Note how discretizing the PSF kernel pushes light out from the center of the profile to approximately 1-2 FWHM:
...
```{r, fig.width=5, fig.height=5, eval=F}
magimage(diffrel,magmap=F,zlim=c(-1,1)*5e-2)
```

Note that our convolution kernel is the integrated flux of a Gaussian in each pixel. This is the correct kernel to use in the trivial case, as it perfectly reproduces the convolution of a point source in the center of the image. However, it cannot exactly convolve a point source located anywhere other than the exact center of the pixel, because that is where the convolution kernel is centered.

Cappellari 2017 state that the use of an integrated convolution kernel (rather than the value of the function at the center of a pixel) is equivalent to an extra convolution by a boxcar filter. But each pixel in the image is convolved as if the flux were entirely contained at the center of the pixel, which is the opposite of convolution by a boxcar filter. The source of the error when discretizing convolution kernels as the integral of the distribution is the fact that barycenter of the flux is not at the center of the pixel. For a monotonically decreasing function (1D or 2D), the barycenter is always shifted from the pixel center slightly towards the peak of the distribution. This means that the convolution pushes the flux out slightly further from the center of the distribution than it should, and hence the resulting image is smoothed slightly more than it should be.
...
One solution suggested by Cappellari 2017 (and commonly used elsewhere, though not always intentionally) is to evaluate the kernel at the center of the pixel instead of integrating it:
...
The answer is yes - by realizing that we don't actually need the finesampled, convolved image, we can reduce the scaling to only finesample^2 convolutions at the original image size with offset PSFs. Behold:
...
Now let's test non-circular Gaussians (not finished, stop here!):
