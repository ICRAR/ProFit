...
Using constraints it is possible to force relative sizes between componenets etc. Here we do not want the bulge Re to become larger than the disk Re.
...
Data=profitSetupData(image=image, mask=mask, sigma=sigma, segim=segim, psf=psf,
  ...
  constraints=constraints, magzero=0, algo.func='optim', like.func = "t", verbose=TRUE)
```
...
First try `optim` L-BFGS-B. If we use the lower and upper limits then these will need to be logged where appropriate (i.e. where specified in the tolog structure). This is because optim only ever sees the logged form of the relevant parameters, the linearisation happens within profitLikeModel.

```{r, eval=FALSE}
sigmas=c(2,2,2,2,5,5,1,1,1,1,30,30,0.3,0.3)
lowers=c(0,0,0,0,10,10,0,0,-0.3,-0.3,-180,-180,-1,-1,-1,-1)
uppers=c(300,300,300,300,30,30,2,2,1.3,1.3,360,360,0,0,1,1)
