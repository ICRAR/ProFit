...
Data=profitSetupData(image=image, mask=mask, sigma=sigma, segim=segim, psf=psf,
  ...
  magzero=0, algo.func='optim', verbose=TRUE, finesample=1L)
```
...
Check it out while using nicer colour maps:

```{r, eval=FALSE}
library("RColorBrewer")
cmap = rev(colorRampPalette(brewer.pal(9,'YlOrBr'))(200))
errcmap = colorRampPalette(brewer.pal(9,'RdYlBu'))(200)
profitLikeModel(optimfit$par,Data,makeplots=TRUE,serscomp=1,cmap=cmap,errcmap=errcmap)
profitLikeModel(optimfit$par,Data,makeplots=TRUE,serscomp=2,cmap=cmap,errcmap=errcmap)
profitLikeModel(optimfit$par,Data,makeplots=TRUE,serscomp='all',cmap=cmap,errcmap=errcmap)
```
...
```{r, eval=FALSE}
profitLikeModel(LAfit$Summary1[,1],Data,makeplots=TRUE,serscomp=1,cmap=cmap,errcmap=errcmap)
profitLikeModel(LAfit$Summary1[,1],Data,makeplots=TRUE,serscomp=2,cmap=cmap,errcmap=errcmap)
profitLikeModel(LAfit$Summary1[,1],Data,makeplots=TRUE,serscomp='all',cmap=cmap,errcmap=errcmap)
```

Other optimizers can be used. One popular algorithm is `CMA-ES` (`Covariance Matrix Adaptation - Evolutionary Strategy`).
...
First make sure `CMA-ES` is installed. We will use a version that supports parallelization and strict runtime limits:
...
It is recommended to use narrower priors than the very broad ones above to speed up convergence:
...
You can also experiment with combining algorithms: `CMA-ES` may be useful for escaping local minima found by other algorithms.
...
```{r, eval=FALSE}
BestLD=magtri(LDfit$Posterior2, samples=500, samptype='ran',inputpar = LAfit$Summary1[,1])
```

Otherwise try:

```{r, eval=FALSE}
BestLD=magtri(LDfit$Posterior1, samples=1000, samptype='end',inputpar = LAfit$Summary1[,1])
```

We can now check our final fit:

```{r, eval=FALSE}
profitLikeModel(BestLD,Data,makeplots=TRUE,serscomp=1,cmap=cmap,errcmap=errcmap)
profitLikeModel(BestLD,Data,makeplots=TRUE,serscomp=2,cmap=cmap,errcmap=errcmap)
profitLikeModel(BestLD,Data,makeplots=TRUE,serscomp='all',cmap=cmap,errcmap=errcmap)
```
...
```{r, eval=FALSE}
profitLikeModel(BestLD,Dataf,makeplots=TRUE,serscomp='all',cmap=cmap,errcmap=errcmap)
```
...
```{r, eval=FALSE}
Dataf$algo.func = "LA"
LAfitf=LaplaceApproximation(profitLikeModel, parm=LAfit$Summary1[,1], Data=Dataf,
  Iterations=1e3, Method='BFGS', CovEst='Identity', sir=FALSE)
```
...
If you run the above for 1e4 iterations (will take several hours), compare posterior2:

```{r, eval=FALSE}
LDfit$Summary2
LDfitf$Summary2

BestLDf=profitMagtri(LDfit$Posterior1, samptype='end',inputpar = LAfitf$Summary1[,1])
par(mfrow=c(1,1))
plot(1,1)
profitLikeModel(BestLDf,Dataf,makeplots=TRUE,serscomp='all',cmap=cmap,errcmap=errcmap)
```

Excuse the intermediate plot above - R still has incompatibilities between the `layout` and `screen` plotting modes.