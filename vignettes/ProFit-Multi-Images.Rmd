...
**ProFit** v2.0.0 and upwards support highly automated multi-band fitting via the **profitMultiBandFound2Fit** and **profitMultiBandDoFit** functions. These require the images provided to be exactly registered and cropped to the desired level. If this is not the case, then lower level functions probably need to be used instead.

As an example we can load in 9 bands of data that come included in the **ProFound** package, and cutout around a bright disk dominated galaxy:
...
We wish to force some degree of smoothness in the parameters, so we make use of the *smooth.parm* function argument where we limit the smoothness to having 3 degrees of freedom:
...
```{r eval=evalglobal}
wave = c(pivwave[grep('VST', pivwave$filter)[1:4],"pivwave"],
         pivwave[grep('VISTA', pivwave$filter)[1:5],"pivwave"])
```

We can now run a highly automated fit, where the only non default option is to run the stacked image **profoundProFound** run using a tolerance of 20 to stop the large spiral being broken up. This will take about 45 minutes to run:
...
```{r eval=evalglobal}
magplot(wave,MultiFit$parm_smooth[grep('mag1',names(MultiFit$parm_smooth))], ylim=c(19,15.5), type='l', col='red', log='x', xlab='Wave / Ang', ylab='Mag')
lines(wave,MultiFit$parm_smooth[grep('mag2',names(MultiFit$parm_smooth))], col='blue')
...
```{r eval=evalglobal}
magplot(wave,MultiFit$parm_smooth[grep('re2',names(MultiFit$parm_smooth))], ylim=c(0,5), type='l', col='blue', log='x', xlab='Wave / Ang', ylab='Re / px')
abline(h=MultiFit$parm_smooth["sersic.re1"], col='red')
legend('bottomright', legend=c('bulge', 'disk'), lty=1, col=c('red','blue'))
```
