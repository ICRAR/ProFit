...
There are 4 sensible looking options it seems. We can extract the brightest of these likely stars to do our rough PSF estimate.
...
gal_modellist = list(sersic=list(xcen=gal_x, ycen=gal_y, mag=gal_mag, re=gal_re, nser=1, ang=gal_ang, axrat=gal_axrat, box=0))
```

And now we make the other inputs we need for our fitting **Data** structure:

```{r}
gal_tofit=list(sersic=list(xcen=TRUE, ycen=TRUE, mag=TRUE, re=TRUE, nser=TRUE, axrat=TRUE, box=FALSE))
