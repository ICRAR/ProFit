...
There are 4 sensible looking options it seems. We can extract the brightest of these likely stars to do our rough PSF estimate. When doing this in anger you might want to consider fitting all 3 stars (or however many make it through the selection cuts above). This gives you a way to marginalise over the uncertainty in the PSF (I discuss PSF uncertainty in some more detail at the end of this vignette).
...
A more complicated alternative is to fit all the good stars (4 in this case) with the same profile simultaneously. Basically we have 4 PSFs distributed around our image, but we want to fit them all with the same PSF, modulo different xcen, ycen and mag:
...
psf_intervals2=list(
  pointsource = list(
    xcen=list(starlist$xcen[1]+c(-5,5), starlist$xcen[2]+c(-5,5), starlist$xcen[3]+c(-5,5), starlist$xcen[4]+c(-5,5)),
    ycen=list(starlist$ycen[1]+c(-5,5), starlist$ycen[2]+c(-5,5), starlist$ycen[3]+c(-5,5), starlist$ycen[4]+c(-5,5)),
    mag=list(starlist$mag[1]+c(-2,2), starlist$mag[2]+c(-2,2), starlist$mag[3]+c(-2,2), starlist$mag[4]+c(-2,2))
  ),
  psf=list(
    moffat=list(
      mag=list(c(-1, 1)),
