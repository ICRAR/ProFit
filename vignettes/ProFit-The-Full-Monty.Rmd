...
```{r}
object_frac=length(which(segim_expand$objects))/length(image)
qnorm(1-object_frac)
```
...
```{r}
starlist=segim_expand$segstats[segim_expand$segstats$axrat>0.9 & segim_expand$segstats$con>0.2 & segim_expand$segstats$con<0.4 & segim_expand$segstats$maj>1.5 & segim_expand$segstats$maj<2 & segim_expand$segstats$SB_N50<23.5 & segim_expand$segstats$asymm<0.4,]
print(starlist)
```
...
```{r}
psf_image=profitCutout(image, loc=starlist[1,c('xcen','ycen')], box=c(31,31))
psf_sigma=profitCutout(sigma, loc=starlist[1,c('xcen','ycen')], box=c(31,31))
psf_segim=profitCutout(segim_expand$segim, loc=starlist[1,c('xcen','ycen')], box=c(31,31))
```

Look at our target star to check we are happy:

```{r, fig.width=5, fig.height=5}
magimage(psf_image$cutim)
magimage(psf_sigma$cutim)
magimage(psf_segim$cutim)
```
...
psf_mag=starlist[1,'flux']
psf_fwhm=starlist[1,'maj']*2
psf_con=1/sqrt(starlist[1,'con'])
...
```{r}
psf_Data=profitSetupData(psf_image$cutim, sigma=psf_sigma$cutim, modellist=psf_modellist, tofit=psf_tofit, tolog=psf_tolog, magzero=30, algo.func='optim', intervals=psf_intervals)
```
...
We need to make our own region map since our object to fit will not be centrally located:

```{r}
psf_region=segim_expand$segim
psf_region[]=segim_expand$segim %in% starlist$segID
```
...
psf_intervals2=list(
  pointsource = list(
    ...
    mag=list(starlist$flux[1]+c(-2,2), starlist$flux[2]+c(-2,2), starlist$flux[3]+c(-2,2))
  ),
  psf=list(
    moffat=list(
      mag=list(c(-1, 1)),
...
for(i in 1:3){
  magimage(profitCutout(image, loc=starlist[i,c('xcen','ycen')], box=c(51,51))$cutim, col=rev(colorRampPalette(brewer.pal(9, "RdYlBu"))(100)[1:50]))
  magimage(profitCutout(image-psf_model_full2, loc=starlist[i,c('xcen','ycen')], box=c(51,51))$cutim, col=rev(colorRampPalette(brewer.pal(9, "RdYlBu"))(100)))
}
```
...
```{r}
gal_image=profitCutout(image, loc=segim_expand$segstats[6,c('xcen','ycen')], box=c(101,101))
gal_sigma=profitCutout(sigma, loc=segim_expand$segstats[6,c('xcen','ycen')], box=c(101,101))
gal_segim=profitCutout(segim_expand$segim, loc=segim_expand$segstats[6,c('xcen','ycen')], box=c(101,101))
```

Check the cutout regions:

```{r, fig.width=5, fig.height=5}
magimage(gal_image$cutim)
magimage(gal_sigma$cutim)
magimage(gal_segim$cutim)
```
...
```{r}
gal_Data=profitSetupData(gal_image$cutim, sigma=gal_sigma$cutim, modellist=gal_modellist, tofit=gal_tofit, tolog=gal_tolog, magzero=30, algo.func='optim', intervals=gal_intervals, psf=psf_model, segim=gal_segim$cutim)
```
